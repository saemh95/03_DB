 ...
 * 찾아달라는 요청이 들어왔다. 누구인가? (국문학과의 '학과코느'는
 * 학과 테이블 (TB_DEPARTMENT)을 조회해서 찾아 내도록 하자
 * */
SELECT STUDENT_NAME FROM TB_STUDENT WHERE SUBSTR(STUDENT_SSN,8 ,1) =2 AND DEPARTMENT_NO ='001' AND ABSENCE_YN = 'Y'; 

--SELECT STUDENT_NAME FROM TB_STUDENT WHERE ABSENCE_YN = 'N' AND SUBSTR(STUDENT_SSN, 8, 1) = 2 AND  DEPARTMENT_NO = (SELECT DEPARTMENT_NO FROM TB_DEPARTMENT WHERE DEPARTMENT_NO = 001);

/*
 * 4. 도서관에서 대출 도서 장기 연체자 들을 찾아 이름을 게시하고자 한다.
 * 그 대상자들의 학번이 다음과 같을 때
...
SELECT STUDENT_NO, STUDENT_NAME FROM TB_STUDENT WHERE STUDENT_NO IN ('A513079', 'A513090', 'A513091', 'A513110', 'A513119') AND STUDENT_NAME IS NOT NULL ORDER BY STUDENT_NAME DESC;
 ...
 * 그럼 춘기술대학교 총장의 이름을 알아낼 수 있는 SQL
 * 
 * */
...
/*
 * 7. 혹시 전산상의 착오로 학과가 지정되어 있지 않은 학생이
 * 있는지 확인하고자 한다.
 * 어떤한 SQL 문장을 사용하면 될 것인지 작성하시오
 * 
 * */
 ...
 * 제외한 재학중인 학생들의 학번, 이름, 주민번호
 * */

SELECT STUDENT_NO, STUDENT_NAME, STUDENT_SSN FROM TB_STUDENT 
WHERE SUBSTR(ENTRANCE_DATE, 6) = 'R-02' AND STUDENT_ADDRESS LIKE '전주%' 
AND ABSENCE_YN IN 'N';

SELECT STUDENT_NO, STUDENT_NAME, STUDENT_SSN FROM TB_STUDENT 
WHERE EXTRACT(YEAR FROM ENTRANCE_DATE) = 2002 AND STUDENT_ADDRESS LIKE '전주%' 
AND ABSENCE_YN IN 'N';
